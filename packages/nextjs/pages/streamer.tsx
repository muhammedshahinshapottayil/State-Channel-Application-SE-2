...
const Streamer: NextPage = () => {
  ...
  function recieveVoucher(clientAddress: string) {
    ...
    async function processVoucher({ data }: { data: Pick<Voucher, "signature"> & { updatedBalance: string } }) {
      ...
      if (!isValidSignature) {
        console.error("Invalid voucher signer detected.");
        return;
      }
  ...
  const provideService = (client: AddressType, wisdom: string) => {
    ...
    const bestVoucher = vouchers[client];

    if (bestVoucher && BigInt(bestVoucher.updatedBalance) < BigInt(duePayment)) {
      console.log(`Cutting off service to client ${client} due to insufficient balance.`);
      return;
    }
